...
<template>
	...
	<x-n-data-table :data="localEmpires" striped class="pt-3">
		<x-n-data-table-column key="empire_name" :title="$t('manage.empires.table.name')">
		...
		<x-n-data-table-column key="empire_faction" :title="$t('manage.empires.table.faction')">
			<template #render-cell="{ rowData }">
				{{ $t("empire.config.factions." + (rowData.empire_faction || "NONE")) }}
			</template>
		</x-n-data-table-column>
